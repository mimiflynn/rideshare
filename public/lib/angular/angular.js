...
function angularInit(element, bootstrap) {
  var elements = [element],
      appElement,
      module,
      names = ['ng:app', 'ng-server', 'x-ng-server', 'data-ng-server'],
 ...
 * the document, but it must be below the `ng-server` definition.
 * 
...
function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      // response is in the cache. the promise api will ensure that to the server code the api is
      // always async
      xhr.onreadystatechange = function() {
        // onreadystatechange might get called multiple times with readyState === 4 on mobile webkit caused by
        // xhrs that are resolved while the server is in the background (see #5426).
